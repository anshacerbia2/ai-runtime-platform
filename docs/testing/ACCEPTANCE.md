...
| G36 | Open app through ATI One mount; existing realm session; missing realm session; malformed callback/logout URI      | Mounted assets/routes work; existing SSO is silent; interactive login escapes frame; redirect/logout URIs match registered client exactly   | Web identity / P1       |
| G37 | Direct upstream request without portal proxy secret; conflicting/default cookies across internal apps; app logout | Direct bypass denied; app cookies are namespaced/path-scoped; ATI One and sibling app sessions remain intact                                | Web security / P1       |
| G38 | Shared UI component states + token scanner + narrow/expanded internal-app viewport                                | No forbidden raw visual values outside token adapter; canonical states accessible; visual regression stable; layouts work embedded/expanded | Frontend quality / P1+  |
